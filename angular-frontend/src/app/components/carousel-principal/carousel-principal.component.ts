...
export class CarouselPrincipalComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  initSlider(): void {
    ...
    for (let i = 0; i < this.slide_data.length; i++) {
      ...
      slide_title.innerHTML = '<h1>' + this.slide_data[i].title + '</h1>';
      ...
      const subheadHTML = '<div class="caption-subhead"><span>' + this.slide_data[i].copy + '</span></div>';
      const subhead = this.renderer.createElement('div');
      subhead.innerHTML = subheadHTML; 
      this.renderer.appendChild(caption, subhead.firstChild as Node); 
      
      const btnLinkHTML = '<a class="btn" href="#">View More</a>';
      const btnLinkContainer = this.renderer.createElement('div'); 
      btnLinkContainer.innerHTML = btnLinkHTML;
      this.renderer.appendChild(caption, btnLinkContainer.firstChild as Node);

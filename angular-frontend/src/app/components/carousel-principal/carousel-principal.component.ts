...
export class CarouselPrincipalComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  @ViewChild('leftCol', { static: false }) leftCol!: ElementRef<HTMLDivElement>;
  @ViewChild('upButton', { static: false }) upButton!: ElementRef<HTMLAnchorElement>;
  ...
  loadBanners(): void {
    ...
    this.subscription.add(
      this.bannerService.getBannersBySection('inicio', false)
        .subscribe({
          next: (banners) => {
            this.banners = banners.filter(banner => banner.activo);
            // Convertir los banners al formato que espera el componente
            this.slide_data = this.banners.map(banner => {
              return {
                src: banner.imagen,
                title: banner.titulo,
                copy: banner.subtitulo || ''
              };
            });
            this.isLoading = false;
            
            // Si hay banners, inicializar el slider después de un pequeño tiempo
            // para asegurar que los datos están cargados
            if (this.slide_data.length > 0) {
              setTimeout(() => {
                this.initSlider();
                this.startAutoplay();
  ...
  ngAfterViewInit(): void {
    // La inicialización se maneja ahora en loadBanners() después de obtener los datos
  }
  ...
  ngOnDestroy(): void {
    ...
    // Cancelar suscripciones a observables
